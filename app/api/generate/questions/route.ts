...
export async function POST(request: NextRequest) {
  try {
    ...
    let questions;
    try {
      questions = await generateQuestions(content.content, count || 5);
    } catch (error) {
    ...
    try {
      ...
      // Still return questions even if storage fails
      console.warn("Failed to store materials, but returning generated questions");
    }
    ...
    return NextResponse.json(
      {
        error: "An unexpected error occurred while generating questions. Please try again.",
      },
      { status: 500 }
    );
  }
